...
- Object.hashCode()
  ...
  - hash = hashCode() 3723923972
  - hash %16 -->{0-15}

- 采用位运算

  - 因为位运算h & (length-1)效率>取模运算
  - 举例：存亿级数量时，位运算耗时5256ms；取模运算耗时23223ms
  - h & (length-1) ，因为length必须是2的指数次幂，所以length-1 通过二进制表示，永远都是尾端以连续1的形式表示：00001111,00000011
    - 当length-1 和hash做与&运算（只有两个为1时才为1，其余为0）时，会保留hash中后x位的1，例如00001111 & 10000011 = 00000011 
  - h & (length-1)  几乎等价于 h % length
  - 数量较大时会触发多次扩容（每次扩容是原来数组的两倍）触发多次rehash-->位运算数量级一下上升，所以用位运算效率体现更明显
...
- HashMap线程不安全？
  ...
  - 体现在：
    ...
    - 死锁
      ...
      - 解释：<https://blog.csdn.net/qq_22158743/article/details/88932176>
